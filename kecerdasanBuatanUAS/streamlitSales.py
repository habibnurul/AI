...
st.write('R Squared (R2) score on test data:', r2_test)
...
Item_Identifier = st.number_input('Item Identifier', min_value=0, max_value=big_mart_data['Item_Identifier'].max())
Item_Weight = st.number_input('Item Weight', min_value=0.0, max_value=big_mart_data['Item_Weight'].max(), value=12.0)
Item_Fat_Content = st.selectbox('Item Fat Content', big_mart_data['Item_Fat_Content'].unique())
Item_Visibility = st.number_input('Item Visibility', min_value=0.0, max_value=big_mart_data['Item_Visibility'].max(), value=0.05)
Item_Type = st.selectbox('Item Type', big_mart_data['Item_Type'].unique())
Item_MRP = st.number_input('Item MRP', min_value=0.0, max_value=big_mart_data['Item_MRP'].max(), value=150.0)
Outlet_Identifier = st.selectbox('Outlet Identifier', big_mart_data['Outlet_Identifier'].unique())
...
Outlet_Size = st.selectbox('Outlet Size', big_mart_data['Outlet_Size'].unique())
Outlet_Location_Type = st.selectbox('Outlet Location Type', big_mart_data['Outlet_Location_Type'].unique())
Outlet_Type = st.selectbox('Outlet Type', big_mart_data['Outlet_Type'].unique())
